      ...
      <div className="flex flex-wrap gap-[2.125rem] lg:px-[2.125rem] px-0 max-w-[120rem] w-full justify-center m-auto">
        {members.map((item, index) => (
          <Link key={item.id} href={`/portfolio/${item.id}`} passHref>
            <div
              className={`w-[18rem] group ${index % 2 ? "md:mt-24" : "md:mt-0"}`}>
              <div className="relative overflow-hidden rounded-lg group-hover:scale-[1.1] group-hover:cursor-pointer transition-all duration-500 aspect-[4/4] ">
                <Image

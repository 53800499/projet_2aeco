  ...
  return (
    <article className="plaquette-card break-inside-avoid rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm transition hover:shadow-md dark:border-dark_border dark:bg-darklight">
      <div className="flex gap-4">
        <div className="h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-slate-100 dark:bg-dark_border">
          {member.photo ? (
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-lg font-bold text-midnight_text dark:text-white">{name}</h3>
          {member.promo && (
          ...
          {member.profession && (
            <p className="mt-1 text-sm font-medium text-midnight_text dark:text-white/90">
              {member.profession}
              {member.fonction_actuelle ? ` — ${member.fonction_actuelle}` : ""}
            </p>
          )}
          {member.employeur_structure && (
            <p className="text-xs text-grey dark:text-white/60">{member.employeur_structure}</p>
          )}
        </div>
      </div>

      <dl className="mt-4 grid gap-1 text-xs text-grey dark:text-white/70">
